    ...
    const char *path, const char *name, const char *base_dir, int attribute_count, const AttrInfo attributes[], CLogManager *clog_manager)
{
  ...
  // 浣跨敤 table_name.table璁板綍涓�涓?琛ㄧ殑鍏冩暟鎹?
  // 鍒ゆ柇琛ㄦ枃浠舵槸鍚﹀凡缁忓瓨鍦?
...
RC Table::create_index(Trx *trx, const char *index_name, const char *attribute_name)
{
  ...
  if (!fs.is_open()) {
    ...
    return RC::IOERR;  // 鍒涘缓绱㈠紩涓?閫斿嚭閿欙紝瑕佸仛杩樺師鎿嶄綔
  }
...
RC Table::delete_record(Trx *trx, Record *record)
{
  ...
  rc = delete_entry_of_indexes(record->data(), record->rid(), false);  // 閲嶅?嶄唬鐮? refer to commit_delete
  if (rc != RC::SUCCESS) {
...
IndexScanner *Table::find_index_for_scan(const ConditionFilter *filter)
{
  ...
  if (composite_condition_filter != nullptr) {
    ...
    for (int i = 0; i < filter_num; i++) {
      ...
      if (scanner != nullptr) {
        return scanner;  // 鍙?浠ユ壘鍒颁竴涓?鏈�浼樼殑锛屾瘮濡傛瘮杈冪?﹀彿鏄?=
      }
    }
  }
  return nullptr;
